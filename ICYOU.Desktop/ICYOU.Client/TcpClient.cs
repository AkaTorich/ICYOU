...
public class TcpClient
{
    ...
    public void Connect()
    {
        try
        {
            Services.DebugLog.Write($"[CLIENT] Подключение к серверу {_serverHost}:{_serverPort}...");
            _client = new System.Net.Sockets.TcpClient();
            _client.Connect(_serverHost, _serverPort);
            _stream = _client.GetStream();
            _running = true;
            Services.DebugLog.Write($"[CLIENT] Подключение успешно, запуск потоков приема...");
            Task.Run(ReceiveLoop);
            Task.Run(PingLoop);
        }
        catch (Exception ex)
        {
            Services.DebugLog.Write($"[CLIENT] ОШИБКА подключения: {ex.Message}");
