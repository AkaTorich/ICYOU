...
chcp 65001 >nul
echo ========================================
echo ICYOU.Android Build Script
echo ========================================
echo.
...
dotnet build ICYOU.SDK\ICYOU.SDK.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
dotnet build ICYOU.Core\ICYOU.Core.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
dotnet build ICYOU.Modules.E2E\ICYOU.Modules.E2E.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
dotnet build ICYOU.Modules.Quote\ICYOU.Modules.Quote.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
dotnet build ICYOU.Modules.LinkPreview\ICYOU.Modules.LinkPreview.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
echo [6/6] Сборка ICYOU.Mobile для %ARCH%...
dotnet build ICYOU.Mobile\ICYOU.Mobile.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
    echo ОШИБКА: Не удалось собрать ICYOU.Mobile
    pause
    exit /b 1
)
...
dotnet publish ICYOU.Mobile\ICYOU.Mobile.csproj -c %CONFIG% -f %FRAMEWORK% -r %ARCH%
if %ERRORLEVEL% NEQ 0 (
...
:INSTALL
REM Вопрос 4: Установить на эмулятор?
echo [Шаг 4] Установить на первый эмулятор в списке?
set /p INSTALL_EMULATOR="(Y/N, по умолчанию N): "
if /i "%INSTALL_EMULATOR%"=="N" (
...
C:\Users\psysh\AppData\Local\Android\Sdk\platform-tools\adb.exe devices
echo.

echo Установка APK на устройство...
C:\Users\psysh\AppData\Local\Android\Sdk\platform-tools\adb.exe install -r "%APK_PATH%"
if %ERRORLEVEL% NEQ 0 (
...
if /i "%LAUNCH_APP%"=="Y" (
    ...
    C:\Users\psysh\AppData\Local\Android\Sdk\platform-tools\adb.exe shell monkey -p com.companyname.icyou.mobile -c android.intent.category.LAUNCHER 1
